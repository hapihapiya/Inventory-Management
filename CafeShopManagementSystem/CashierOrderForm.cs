    ...
    public partial class CashierOrderForm : UserControl
    {
        ...
        private void cashierOrderForm_addBtn_Click(object sender, EventArgs e)
        {
            ...
            if (cashierOrderForm_type.SelectedIndex == -1 || cashierOrderForm_productID.SelectedIndex == -1
                || cashierOrderForm_prodName.Text == "" || cashierOrderForm_quantity_.Value == 0
                ...
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(insertOrder, connect))
                        {
                            ...
                            float totalPrice = (getPrice * (int)cashierOrderForm_quantity_.Value);

                            cmd.Parameters.AddWithValue("@qty", cashierOrderForm_quantity_.Value);
        ...
        private void cashierOrderForm_quantity(object sender, EventArgs e)
        {
                    ...
                    , MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (connect.State == ConnectionState.Closed)
                    {
                        try
                        {
                            ...
                            string insertData = "INSERT INTO customers (customer_id, total_price, date) " +
                                                 "VALUES (@custID, @totalprice, @date)";
        ...
        public void clearFields()
        {
            ...
            cashierOrderForm_prodName.Text = "";
            cashierOrderForm_price.Text = "";
            cashierOrderForm_quantity_.Value = 0;
        }
